...
class TestDataFrameApply:
    ...
    def test_apply_noreduction_tzaware_object(self):
        ...
        df = pd.DataFrame({"foo": [pd.Timestamp("2020", tz="UTC")]}, dtype="datetime64[ns, UTC]")
        result = df.apply(lambda x: x)
        tm.assert_frame_equal(result, df)
    ...
    def test_apply_with_byte_string(self):
        ...
        expected = pd.DataFrame(np.array([b"abcd", b"efgh"]), columns=["col"], dtype=object)
    ...
    def test_apply_category_equalness(self, val):
        ...
        expected = pd.Series([np.NaN if pd.isnull(x) else x == val for x in df_values], name="a")
        tm.assert_series_equal(result, expected)
