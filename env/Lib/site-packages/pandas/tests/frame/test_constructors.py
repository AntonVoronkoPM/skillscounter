...
class TestDataFrameConstructors:
    ...
    def test_from_dict_columns_parameter(self):
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame.from_dict(dict([("A", [1, 2]), ("B", [4, 5])]), columns=["one", "two"])
    ...
    def test_from_records_series_categorical_index(self):
        # GH 32805
        index = CategoricalIndex([pd.Interval(-20, -10), pd.Interval(-10, 0), pd.Interval(0, 10)])
    ...
    def test_construct_with_two_categoricalindex_series(self):
        # GH 14600
        s1 = Series([39, 6, 4], index=pd.CategoricalIndex(["female", "male", "unknown"]))
        s2 = Series(
            [2, 152, 2, 242, 150],
        ...
        expected = DataFrame(
            np.array([[np.nan, 39.0, np.nan, 6.0, 4.0], [2.0, 152.0, 2.0, 242.0, 150.0]]),
