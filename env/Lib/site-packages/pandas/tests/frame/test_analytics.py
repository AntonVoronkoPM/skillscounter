...
class TestDataFrameAnalytics:
    ...
    def test_sum_prod_nanops(self, method, unit):
        idx = ["a", "b", "c"]
        df = pd.DataFrame({"a": [unit, unit], "b": [unit, np.nan], "c": [np.nan, np.nan]})
        # The default
    ...
    def test_sum_mixed_datetime(self):
        # GH#30886
        df = pd.DataFrame({"A": pd.date_range("2000", periods=4), "B": [1, 2, 3, 4]}).reindex(
            [2, 3, 4]
        )
        result = df.sum()
    ...
    def test_any_all_level_axis_none_raises(self, method):
        df = DataFrame(
            {"A": 1},
            index=MultiIndex.from_product([["A", "B"], ["a", "b"]], names=["out", "in"]),
        )
...
def test_minmax_extensionarray(method, numeric_only):
    ...
    expected = Series([getattr(int64_info, method)], index=pd.Index(["Int64"], dtype="object"))
    tm.assert_series_equal(result, expected)
