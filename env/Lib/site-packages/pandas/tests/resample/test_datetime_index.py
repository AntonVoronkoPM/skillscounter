...
def test_resample_integerarray():
    ...
    ts = pd.Series(range(9), index=pd.date_range("1/1/2000", periods=9, freq="T"), dtype="Int64")
...
def test_resample_anchored_multiday():
    ...
    index = pd.date_range("2014-10-14 23:06:23.206", periods=3, freq="400L") | pd.date_range(
...
def test_groupby_with_dst_time_change():
    # GH 24972
    index = pd.DatetimeIndex([1478064900001000000, 1480037118776792000], tz="UTC").tz_convert(
        "America/Chicago"
    )
...
def test_resample_dst_anchor():
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            ...
            index=date_range("9/30/2013", "11/11/2013", freq="2W-MON", tz="Europe/Paris"),
        ),
        "2W-MON Frequency",
    )
...
def test_downsample_across_dst_weekly():
    ...
    expected = DataFrame(
        [23, 42],
        index=pd.DatetimeIndex(["2017-03-26", "2017-04-02"], tz="Europe/Amsterdam", freq="W"),
    )
    tm.assert_frame_equal(result, expected)
...
def test_resample_equivalent_offsets(n1, freq1, n2, freq2, k):
    ...
    n1_ = n1 * k
    n2_ = n2 * k
    s = pd.Series(0, index=pd.date_range("19910905 13:00", "19911005 07:00", freq=freq1))
    s = s + range(len(s))
...
def test_resample_apply_product():
    ...
    expected = DataFrame(
        ...
        index=DatetimeIndex(["2012-03-31", "2012-06-30", "2012-09-30", "2012-12-31"], freq="Q-DEC"),
        columns=["A", "B"],
    )
    tm.assert_frame_equal(result, expected)
