...
class TestHDFStore:
    ...
    def test_append_with_data_columns(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            tm.assert_frame_equal(result, expected, check_index_type=False, check_freq=False)
    ...
    @pytest.mark.skipif(
        ...
        reason=(
            "Skipping  pytables test when numpy version is " "exactly equal to 1.15.0: gh-22098"
        ),
    )
    ...
    def test_retain_index_attributes(self, setup_path):
        ...
        with ensure_clean_store(setup_path) as store:
            ...
            with catch_warnings(record=True):
                df2 = DataFrame(
                    dict(A=Series(range(3), index=date_range("2002-1-1", periods=3, freq="D")))
                )
