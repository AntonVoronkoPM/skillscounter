...
def test_append_with_timezones_dateutil(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(
            dict(
                A=[
                    Timestamp("20130102 2:00:00", tz=gettz("US/Eastern")) + timedelta(hours=1) * i
                    for i in range(5)
                ]
            )
        )
...
def test_append_with_timezones_pytz(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(
            dict(
                A=[
                    Timestamp("20130102 2:00:00", tz="US/Eastern") + timedelta(hours=1) * i
                    for i in range(5)
                ]
            )
        )
...
def test_legacy_datetimetz_object(datapath, setup_path):
    ...
    # 8260
    expected = DataFrame(
        dict(A=Timestamp("20130102", tz="US/Eastern"), B=Timestamp("20130603", tz="CET")),
        index=range(5),
    )
