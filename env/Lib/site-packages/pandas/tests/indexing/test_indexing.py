...
class TestFancy:
    ...
    def test_setitem_ndarray_3d(self, index, obj, idxr, idxr_id):
        ...
        elif isinstance(index, pd.IntervalIndex) and idxr_id == "setitem" and obj.ndim == 1:
            err = AttributeError
...
def test_extension_array_cross_section():
    ...
    expected = pd.Series(pd.core.arrays.integer_array([1, 3]), index=["A", "B"], name="a")
    result = df.loc["a"]
    tm.assert_series_equal(result, expected)
...
def test_extension_array_cross_section_converts():
    ...
    df = pd.DataFrame(
        {"A": pd.array([1, 2], dtype="Int64"), "B": np.array([1, 2])},
        index=["a", "b"],
    )
    result = df.loc["a"]
...
def test_setitem_categorical():
    ...
    expected = pd.DataFrame({"h": pd.Categorical(["m", "n"]).reorder_categories(["n", "m"])})
    tm.assert_frame_equal(df, expected)
