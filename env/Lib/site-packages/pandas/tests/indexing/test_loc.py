...
class TestLoc2:
    ...
    def test_loc_to_fail(self):
        ...
        # raise a KeyError?
        msg = r"\"None of \[Int64Index\(\[1, 2\], dtype='int64'\)\] are " r"in the \[index\]\""
        ...
        s["a"] = 2
        msg = r"\"None of \[Int64Index\(\[-2\], dtype='int64'\)\] are " r"in the \[index\]\""
        with pytest.raises(KeyError, match=msg):
            s.loc[[-2]]
        ...
        msg = r"\"None of \[Int64Index\(\[3\], dtype='int64'\)\] are " r"in the \[index\]\""
        with pytest.raises(KeyError, match=msg):
            df.loc[[3], :]
    ...
    def test_loc_getitem_list_with_fail(self):
        ...
        with pytest.raises(
            KeyError,
            match=re.escape("\"None of [Int64Index([3], dtype='int64')] are in the [index]\""),
        ):
            s.loc[[3]]
...
Region_1,Site_2,3977723089,A,5/20/2015 8:33,5/20/2015 9:09,Yes,No"""
        ...
        df.loc[:, ("Respondent", "Duration")] = df.loc[:, ("Respondent", "Duration")].astype(
            "timedelta64[s]"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                pd.DataFrame([[7, 2, 8], [9, 4, 10], [11, 6, 12]], columns=["A", "B", "C"]),
            ),
        ],
    )
    ...
    def test_loc_setitem_empty_append_raises(self):
        ...
        msg = r"None of \[Int64Index\(\[0, 1\], dtype='int64'\)\] " r"are in the \[index\]"
        with pytest.raises(KeyError, match=msg):
            df.loc[[0, 1], "x"] = data
...
def test_loc_axis_1_slice():
    ...
    expected = pd.DataFrame(
        np.ones((10, 4)),
        index=tuple("ABCDEFGHIJ"),
        columns=pd.MultiIndex.from_tuples([(2014, 9), (2014, 10), (2015, 7), (2015, 8)]),
    )
    tm.assert_frame_equal(result, expected)
