...
def test_quantile_array2():
    ...
    df = pd.DataFrame(np.random.RandomState(0).randint(0, 5, size=(10, 3)), columns=list("ABC"))
...
def test_quantile_array_multiple_levels():
    df = pd.DataFrame({"A": [0, 1, 2], "B": [3, 4, 5], "c": ["a", "a", "a"], "d": ["a", "a", "b"]})
    result = df.groupby(["c", "d"]).quantile([0.25, 0.75])
    index = pd.MultiIndex.from_tuples(
        [("a", "a", 0.25), ("a", "a", 0.75), ("a", "b", 0.25), ("a", "b", 0.75)],
        names=["c", "d", None],
    )
    expected = pd.DataFrame({"A": [0.25, 0.75, 2.0, 2.0], "B": [3.25, 3.75, 5.0, 5.0]}, index=index)
    tm.assert_frame_equal(result, expected)
...
def test_groupby_quantile_with_arraylike_q_and_int_columns(frame_size, groupby, q):
    ...
    df = pd.DataFrame(np.array([ncol * [_ % 4] for _ in range(nrow)]), columns=range(ncol))
    ...
    expected = pd.DataFrame(expected_values, index=expected_index, columns=expected_columns)
...
def test_quantile_raises():
    df = pd.DataFrame([["foo", "a"], ["foo", "b"], ["foo", "c"]], columns=["key", "val"])
...
def test_quantile_missing_group_values_correct_results(key, val, expected_key, expected_val):
    ...
    expected = pd.DataFrame(expected_val, index=pd.Index(expected_key, name="key"), columns=["val"])
