...
def test_series_fast_transform_date():
    # GH 13191
    df = pd.DataFrame({"grouping": [np.nan, 1, 1, 3], "d": pd.date_range("2014-1-1", "2014-1-4")})
...
def test_groupby_transform_with_nan_group():
    ...
    expected = pd.Series([1.0, 1.0, 2.0, 3.0, np.nan, 6.0, 6.0, 9.0, 9.0, 9.0], name="a")
    tm.assert_series_equal(result, expected)
...
def test_transform_with_non_scalar_group():
    ...
    df = pd.DataFrame(np.random.randint(1, 10, (4, 12)), columns=cols, index=["A", "C", "G", "T"])
...
def test_any_all_np_func(func):
    # GH 20653
    df = pd.DataFrame([["foo", True], [np.nan, True], ["foo", True]], columns=["key", "val"])
...
@pytest.mark.parametrize(
    "obj",
    [
        DataFrame(dict(a=[0, 0, 0, 1, 1, 1], b=range(6)), index=["A", "B", "C", "D", "E", "F"]),
