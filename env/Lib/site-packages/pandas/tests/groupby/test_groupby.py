...
def test_groupby_keys_same_size_as_index():
    # GH 11185
    freq = "s"
    index = pd.date_range(start=pd.Timestamp("2015-09-29T11:34:44-0700"), periods=2, freq=freq)
...
def test_groupby_multiindex_not_lexsorted():
    ...
    for level in [0, 1, [0, 1]]:
        for sort in [False, True]:
            ...
            result = (
                df.sort_index().groupby(level=level, sort=sort).apply(DataFrame.drop_duplicates)
            )
