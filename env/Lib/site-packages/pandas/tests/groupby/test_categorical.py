...
@pytest.mark.parametrize("operation, kwargs", [("agg", dict(dtype="category")), ("apply", dict())])
...
def test_groupby_cat_preserves_structure(observed, ordered):
    ...
    result = df.groupby("Name", observed=observed).agg(pd.DataFrame.sum, skipna=True).reset_index()
...
def test_groupby_agg_non_numeric():
    df = pd.DataFrame({"A": pd.Categorical(["a", "a", "b"], categories=["a", "b", "c"])})
...
def test_agg_cython_category_not_implemented_fallback():
    ...
    expected = pd.Series([1, 2, 3], index=pd.Index([1, 2, 3], name="col_num"), name="col_cat")
    tm.assert_series_equal(result, expected)
