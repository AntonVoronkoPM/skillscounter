...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(lambda x: x.copy(deep=True), marks=pytest.mark.xfail(reason="GH-34998")),
    ],
)
...
def test_apply_reindex_values():
    ...
    values = [1, 2, 3, 4]
    indices = [1, 1, 2, 2]
    df = pd.DataFrame({"group": ["Group1", "Group2"] * 2, "value": values}, index=indices)
...
def test_apply_aggregating_timedelta_and_datetime():
    ...
    result = df.groupby("clientid").apply(
        lambda ddf: pd.Series(dict(clientid_age=ddf.time_delta_zero.min(), date=ddf.datetime.min()))
    )
...
def test_apply_multi_level_name(category):
    ...
    expected = pd.DataFrame({"C": [20, 25], "D": [20, 25]}, index=pd.Index([1, 2], name="B"))
    tm.assert_frame_equal(result, expected)
