...
def test_agg_timezone_round_trip():
    ...
    df = pd.DataFrame({"a": 1, "b": [ts + dt.timedelta(minutes=nn) for nn in range(10)]})
...
def test_agg_structs_dataframe(structure, expected):
    df = pd.DataFrame({"A": [1, 1, 1, 3, 3, 3], "B": [1, 1, 1, 4, 4, 4], "C": [1, 1, 1, 3, 4, 4]})
...
def test_agg_category_nansum(observed):
    categories = ["a", "b", "c"]
    df = pd.DataFrame({"A": pd.Categorical(["a", "a", "b"], categories=categories), "B": [1, 2, 3]})
...
def test_agg_list_like_func():
    # GH 18473
    df = pd.DataFrame({"A": [str(x) for x in range(3)], "B": [str(x) for x in range(3)]})
    grouped = df.groupby("A", as_index=False, sort=False)
    result = grouped.agg({"B": lambda x: list(x)})
    expected = pd.DataFrame({"A": [str(x) for x in range(3)], "B": [[str(x)] for x in range(3)]})
    tm.assert_frame_equal(result, expected)
