...
def test_agg_apply_corner(ts, tsframe):
    ...
    exp_df = DataFrame(columns=tsframe.columns, dtype=float, index=pd.Index([], dtype=np.float64))
...
def test_aggregate_item_by_item(df):
    ...
    exp = pd.Series(np.array([foo] * K), index=list("BCD"), dtype=np.float64, name="foo")
    tm.assert_series_equal(result.xs("foo"), exp)

    exp = pd.Series(np.array([bar] * K), index=list("BCD"), dtype=np.float64, name="bar")
...
class TestNamedAggregationDataFrame:
    ...
    def test_duplicate_no_raises(self):
        ...
        expected = pd.DataFrame({"a": [1, 3], "b": [1, 3]}, index=pd.Index([0, 1], name="A"))
...
def test_aggregate_mixed_types():
    # GH 16916
    df = pd.DataFrame(data=np.array([0] * 9).reshape(3, 3), columns=list("XYZ"), index=list("abc"))
