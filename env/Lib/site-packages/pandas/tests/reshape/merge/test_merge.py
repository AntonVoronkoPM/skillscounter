...
class TestMerge:
    ...
    def test_merge_preserves_row_order(self, how):
        ...
        if how == "right":
            expected = pd.DataFrame({"animal": ["quetzal", "pig"], "max_speed": [80, 11]})
        else:
...
def test_merge_series(on, left_on, right_on, left_index, right_index, nm):
    ...
    a = pd.DataFrame(
        {"A": [1, 2, 3, 4]},
        index=pd.MultiIndex.from_product([["a", "b"], [0, 1]], names=["outer", "inner"]),
    )
    b = pd.Series(
        [1, 2, 3, 4],
        index=pd.MultiIndex.from_product([["a", "b"], [1, 2]], names=["outer", "inner"]),
        name=nm,
    )
    expected = pd.DataFrame(
        {"A": [2, 4], "B": [1, 3]},
...
def test_merge_datetime_upcast_dtype():
    ...
    df2 = pd.DataFrame({"y": ["1", "2", "3"], "z": pd.to_datetime(["2000", "2001", "2002"])})
