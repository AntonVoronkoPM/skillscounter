...
class TestPivotTable:
    ...
    def test_pivot_with_non_observable_dropna(self, dropna):
        ...
        expected = pd.DataFrame(
            {"B": [2, 3]},
            index=pd.Index(
                pd.Categorical.from_codes([0, 1], categories=["low", "high"], ordered=True),
                name="A",
            ),
        )
        ...
        df = DataFrame(
            {
                ...
                "B": ["a", "b", "a", "b"],
                "C": pd.Categorical(ordered_cat, ordered=True).sort_values(ascending=False),
            }
        )
    ...
    def test_pivot_preserve_dtypes(self, columns, values):
        ...
        expected = {
            col: np.dtype("O") if col[0].startswith("b") else np.dtype("float64") for col in df_res
        }
    ...
    def test_pivot_no_values(self):
        ...
        exp = pd.DataFrame([[2.5, 4.0], [2.0, np.nan]], index=[1, 2], columns=exp_columns)
        ...
        res = df.pivot_table(index=df.index.month, columns=pd.Grouper(key="dt", freq="M"))
        ...
        res = df.pivot_table(index=pd.Grouper(freq="A"), columns=pd.Grouper(key="dt", freq="M"))
        exp = pd.DataFrame(
    ...
    def test_pivot_periods_with_margins(self):
        ...
        expected = DataFrame(
            ...
            columns=pd.Index([pd.Period("2019Q1"), pd.Period("2019Q2"), "All"], name="b"),
        )
    ...
    def test_margin_with_only_columns_defined(self, columns, aggfunc, values, expected_columns):
        ...
        expected = pd.DataFrame(values, index=Index(["D", "E"]), columns=expected_columns)
