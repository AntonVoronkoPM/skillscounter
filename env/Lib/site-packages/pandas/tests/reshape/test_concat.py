...
class TestAppend:
    ...
    def test_append_empty_tz_frame_with_datetime64ns(self):
        ...
        df = pd.DataFrame(columns=["a"]).astype("datetime64[ns, UTC]")
        result = df.append(pd.Series({"a": pd.NaT}, dtype="datetime64[ns]"), ignore_index=True)
        expected = pd.DataFrame({"a": [pd.NaT]}).astype("datetime64[ns]")
        tm.assert_frame_equal(result, expected)
...
class TestConcatenate:
    ...
    def test_concat_series_axis1_names_applied(self):
        ...
        expected = DataFrame([[1, 4], [2, 5], [3, 6]], columns=pd.Index(["a", "b"], name="A"))
        tm.assert_frame_equal(result, expected)
...
"""
    ...
    def test_concat_datetime_timezone(self):
        ...
        expected = pd.DataFrame([[1, 1], [2, 2], [3, 3]], index=exp_idx, columns=["a", "b"])
    ...
    def test_concat_odered_dict(self):
        ...
        result = pd.concat(
            OrderedDict([("First", pd.Series(range(3))), ("Another", pd.Series(range(4)))])
        )
        tm.assert_series_equal(result, expected)
...
def test_concat_sparse():
    ...
    expected = pd.DataFrame(data=[[0, 0], [1, 1], [2, 2]]).astype(pd.SparseDtype(np.int64, 0))
