...
def _is_potential_multi_index(columns, index_col: Optional[Union[bool, Sequence[int]]] = None):
    """
    ...
    def _extract_multi_indexer_columns(self, header, index_names, col_names, passed_names=False):
        ...
        for n in range(len(columns[0])):
            if all(ensure_str(col[n]) in self.unnamed_cols for col in columns):
                ...
                raise ParserError(
                    f"Passed header=[{header}] are too many rows " "for this multi_index of columns"
                )
    ...
    def _convert_to_ndarrays(
        ...
        for c, values in dct.items():
            ...
            if conv_f is not None:
                ...
                is_str_or_ea_dtype = is_string_dtype(cast_type) or is_extension_array_dtype(
                    cast_type
                )
    ...
    def __init__(self, f, **kwds):
        ...
        if len(self.columns) > 1:
            ...
            (
                ...
                self.col_names,
                _,
            ) = self._extract_multi_indexer_columns(self.columns, self.index_names, self.col_names)
        ...
        if self.thousands is None:
            ...
            self.nonnum = re.compile(rf"[^-^0-9^{self.thousands}^{self.decimal}]+")
        ...
        if self.header is not None:
            ...
            for level, hr in enumerate(header):
                try:
                    ...
                    if self.line_pos < hr:
                        raise ValueError(
                            f"Passed header={hr} but only {self.line_pos + 1} lines in " "file"
                        ) from err
        ...
        if self.usecols:
            if self._implicit_index:
                zipped_content = [
                    ...
                    if (i < len(self.index_col) or i - len(self.index_col) in self._col_indices)
                ]
            else:
