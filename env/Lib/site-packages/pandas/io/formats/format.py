    ...
    def _to_str_columns(self) -> List[List[str]]:
        ...
        if not is_list_like(self.header) and not self.header:
            ...
            for i, c in enumerate(frame):
                ...
                header_colwidth = max(col_space.get(c, 0), *(self.adj.len(x) for x in cheader))
                fmt_values = self._format_col(i)
                fmt_values = _make_fixed_width(
...
def format_percentiles(
    percentiles: Union[np.ndarray, List[Union[int, float]], List[float], List[Union[str, float]]]
) -> List[str]:
    """
...
def _is_dates_only(values: Union[np.ndarray, DatetimeArray, Index, DatetimeIndex]) -> bool:
...
def _trim_zeros_complex(
    ...
    """
    return [
        "".join(_trim_zeros_float(re.split(r"([j+-])", x), decimal, na_rep)) for x in str_complexes
    ]
