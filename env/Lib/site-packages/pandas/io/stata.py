...
def _datetime_to_stata_elapsed_vec(dates: Series, fmt: str) -> Series:
    ...
    def parse_dates_safe(dates, delta=False, year=False, days=False):
        ...
        if is_datetime64_dtype(dates.dtype):
            ...
            if days:
                days_in_ns = dates.astype(np.int64) - to_datetime(d["year"], format="%Y").astype(
                    np.int64
                )
...
class StataReader(StataParser, abc.Iterator):
    ...
    def __init__(
        ...
        if self._chunksize is not None and (not isinstance(chunksize, int) or chunksize <= 0):
...
the string values returned are correct."""
    ...
    def __next__(self) -> DataFrame:
        if self._chunksize is None:
            raise ValueError("chunksize must be set to a positive integer to use as an iterator.")
