...
class DataFrame(NDFrame):
    ...
    def to_dict(self, orient="dict", into=dict):
        ...
        if orient == "dict":
            ...
            return [
                into_c((k, com.maybe_box_datetimelike(v)) for k, v in row.items()) for row in rows
            ]
            ...
            return into_c(
                (t[0], dict(zip(self.columns, t[1:]))) for t in self.itertuples(name=None)
            )
    ...
    def to_stata(
        ...
        if version == 114:
            ...
            from pandas.io.stata import (
                StataWriter117 as statawriter,  # type: ignore
            )
        else:  # versions 118 and 119
            # mypy: Name 'statawriter' already defined (possibly by an import)
            from pandas.io.stata import (
                StataWriterUTF8 as statawriter,  # type: ignore
            )
    ...
    def info(
        ...
        return DataFrameInfo(self, verbose, buf, max_cols, memory_usage, null_counts).info()
    ...
    def _sanitize_column(self, key, value, broadcast=True):
        ...
        if isinstance(value, Series):
            ...
            if is_extension_array_dtype(infer_dtype):
                value = construct_1d_arraylike_from_scalar(value, len(self.index), infer_dtype)
            else:
    ...
    def _replace_columnwise(self, mapping: Dict[Label, Tuple[Any, Any]], inplace: bool, regex):
        """
    ...
    def reset_index(
        ...
        if not drop:
            ...
            if isinstance(self.index, MultiIndex):
                names = [
                    (n if n is not None else f"level_{i}") for i, n in enumerate(self.index.names)
                ]
...
"""
    ...
    def _reduce(
        ...
        **kwds,
        ...
        dtype_is_dt = np.array(
            [is_datetime64_any_dtype(values.dtype) for values in self._iter_column_arrays()],
            dtype=bool,
        )
        ...
        if not self._is_homogeneous_type or self._mgr.any_extension_types:
            ...
            if filter_type is None and axis == 0 and len(self) > 0:
                ...
                opa = frame_apply(self, func=func, result_type="expand", ignore_failures=True)
    ...
    index: "Index" = properties.AxisProperty(axis=1, doc="The index (row labels) of the DataFrame.")
    columns: "Index" = properties.AxisProperty(axis=0, doc="The column labels of the DataFrame.")
