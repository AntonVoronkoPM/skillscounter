    ...
    def _prep_values(self, values: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        if is_float_dtype(values.dtype):
            ...
            raise NotImplementedError(
                f"ops for {self._window_type} for this " f"dtype {values.dtype} are not implemented"
            )
        else:
            try:
                values = ensure_float64(values)
    ...
    **kwargs
    ...
    def cov(self, other=None, pairwise=None, ddof=1, **kwargs):
        ...
        def _get_cov(X, Y):
            ...
            count = (
                (X + Y).rolling(window=window, min_periods=0, center=self.center).count(**kwargs)
            )
...
class RollingGroupby(WindowGroupByMixin, Rolling):
    ...
    def _apply(
        ...
        **kwargs,
        ...
        result_index = MultiIndex.from_tuples(result_index_data, names=result_index_names)
    ...
    def _create_blocks(self, obj: FrameOrSeries):
        ...
        # GH 36197
        if not obj.empty:
            groupby_order = np.concatenate(list(self._groupby.grouper.indices.values())).astype(
                np.int64
            )
