...
class BlockManager(PandasObject):
    ...
    def __getstate__(self):
        ...
        extra_state = {
            "0.14.1": {
                "axes": axes_array,
                "blocks": [dict(values=b.values, mgr_locs=b.mgr_locs.indexer) for b in self.blocks],
            }
        }
    ...
    def iget(self, i: int) -> "SingleBlockManager":
        ...
        return SingleBlockManager(
            block.make_block_same_class(values, placement=slice(0, len(values)), ndim=1),
            self.axes[1],
        )
    ...
    try:
        if len(blocks) == 1 and not isinstance(blocks[0], Block):
            ...
            if not len(blocks[0]):
                ...
                blocks = [make_block(values=blocks[0], placement=slice(0, len(axes[0])))]
...
def create_block_manager_from_arrays(arrays, names: Index, axes: List[Index]) -> BlockManager:
...
def _merge_blocks(blocks: List[Block], dtype: DtypeObj, can_consolidate: bool) -> List[Block]:
