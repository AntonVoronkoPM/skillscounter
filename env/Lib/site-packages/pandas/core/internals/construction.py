...
def masked_rec_array_to_mgr(data, index, columns, dtype: Optional[DtypeObj], copy: bool):
    """
...
def init_dict(data: Dict, index, columns, dtype: Optional[DtypeObj] = None):
    ...
    if columns is not None:
        ...
        if missing.any() and not is_integer_dtype(dtype):
            if dtype is None or (
                not is_extension_array_dtype(dtype) and np.issubdtype(dtype, np.flexible)
            ):
                # GH#1783
        ...
        arrays = [arr if not isinstance(arr, ABCIndexClass) else arr._data for arr in arrays]
...
def to_arrays(data, columns, coerce_float: bool = False, dtype: Optional[DtypeObj] = None):
    ...
    if isinstance(data[0], (list, tuple)):
        ...
        return data, columns
    elif isinstance(data, (np.ndarray, ABCSeries, Index)) and data.dtype.names is not None:
