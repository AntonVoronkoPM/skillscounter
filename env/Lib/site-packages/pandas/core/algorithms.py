...
import numpy as np
from pandas._libs import Timestamp, algos
...
# dtype access    #
# --------------- #
def _ensure_data(values, dtype: Optional[DtypeObj] = None) -> Tuple[np.ndarray, DtypeObj]:
    """
    ...
    elif is_categorical_dtype(vals_dtype) and (is_categorical_dtype(dtype) or dtype is None):
        values = values.codes
...
def checked_add_with_arr(arr, b, arr_mask=None, b_mask=None):
    ...
    if not mask1.any():
        ...
        to_raise = ((np.iinfo(np.int64).max - b2[mask1] < arr[mask1]) & not_nan[mask1]).any() or (
...
def take_nd(arr, indexer, axis: int = 0, out=None, fill_value=np.nan, allow_fill: bool = True):
    """
