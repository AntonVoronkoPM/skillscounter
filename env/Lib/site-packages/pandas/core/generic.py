...
def _single_replace(self, to_replace, method, inplace, limit):
    ...
    if self.ndim != 1:
        raise TypeError(
            f"cannot replace {to_replace} with method {method} on a " f"{type(self).__name__}"
        )
...
class NDFrame(PandasObject, SelectionMixin, indexing.IndexingMixin):
    ...
    def _AXIS_NAMES(self) -> Dict[int, str]:
        """.. deprecated:: 1.1.0"""
        warnings.warn(
            "_AXIS_NAMES has been deprecated.",
            FutureWarning,
            stacklevel=3,
        )
        return {0: "index"}
    ...
    @classmethod
    def _construct_axes_from_arguments(cls, args, kwargs, require_all: bool = False, sentinel=None):
        """
    ...
    def __hash__(self):
        raise TypeError(
            f"{repr(type(self).__name__)} objects are mutable, " f"thus they cannot be hashed"
        )
    ...
    def infer_objects(self: FrameOrSeries) -> FrameOrSeries:
        ...
        return self._constructor(
            self._mgr.convert(datetime=True, numeric=False, timedelta=True, coerce=False, copy=True)
    ...
    def convert_dtypes(
        ...
        if self.ndim == 1:
            ...
            results = [
                col._convert_dtypes(infer_objects, convert_string, convert_integer, convert_boolean)
                for col_name, col in self.items()
            ]
            result = pd.concat(results, axis=1, copy=False)
            return result
    ...
    def tshift(self: FrameOrSeries, periods: int = 1, freq=None, axis: Axis = 0) -> FrameOrSeries:
        """
    ...
    def _add_numeric_operations(cls):
        ...
        @doc(
            desc="Return the mean absolute deviation of the values " "for the requested axis.",
            name1=name1,
            name2=name2,
            axis_descr=axis_descr,
            see_also="",
...
def _make_stat_function(
    ...
    @Appender(_num_doc)
    def stat_func(self, axis=None, skipna=None, level=None, numeric_only=None, **kwargs):
        if name == "median":
...
def _make_stat_function_ddof(
    ...
    def stat_func(self, axis=None, skipna=None, level=None, ddof=1, numeric_only=None, **kwargs):
