...
def _box_as_indexlike(dt_array: ArrayLike, utc: Optional[bool] = None, name: Label = None) -> Index:
    """
...
def _adjust_to_origin(arg, origin, unit):
    ...
    if origin == "julian":
        ...
        if np.any(arg > j_max) or np.any(arg < j_min):
            raise tslibs.OutOfBoundsDatetime(f"{original} is Out of Bounds for origin='julian'")
    else:
        # arg must be numeric
        if not (
        ...
        try:
            ...
            raise tslibs.OutOfBoundsDatetime(f"origin {origin} is Out of Bounds") from err
