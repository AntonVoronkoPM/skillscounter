        ...
        **kwargs : kwargs to pass on to the function
        ...
        if isinstance(arg, dict):
            ...
            if any(is_aggregator(x) for x in arg.values()):
                ...
                # GH 15931
                keys = list(arg.keys())
                if isinstance(obj, ABCDataFrame) and len(obj.columns.intersection(keys)) != len(
                    keys
                ):
...
class IndexOpsMixin:
    ...
    def duplicated(self, keep="first"):
        if isinstance(self, ABCIndexClass):
            ...
            return self._constructor(duplicated(self, keep=keep), index=self.index).__finalize__(
