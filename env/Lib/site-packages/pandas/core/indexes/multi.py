...
_index_doc_kwargs.update(dict(klass="MultiIndex", target_klass="MultiIndex or list of tuples"))
...
class MultiIndex(Index):
        ...
        for i, k in enumerate(seq):
            ...
            if com.is_bool_indexer(k):
                ...
                indexer = _update_indexer(
                    _convert_to_indexer(self._get_level_indexer(k, level=i, indexer=indexer)),
                    indexer=indexer,
                )
            else:
                # a single label
                indexer = _update_indexer(
                    _convert_to_indexer(self.get_loc_level(k, level=i, drop_level=False)[0]),
                    indexer=indexer,
                )
    ...
    def astype(self, dtype, copy=True):
        ...
        if is_categorical_dtype(dtype):
            ...
            raise TypeError(
                f"Setting {type(self)} dtype to anything other " "than object is not supported"
            )
        elif copy is True:
            return self._shallow_copy()
        return self
