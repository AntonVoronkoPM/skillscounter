...
class Series(base.IndexOpsMixin, generic.NDFrame):
    ...
    def view(self, dtype=None) -> "Series":
        ...
        dtype: int8
        """
        return self._constructor(self._values.view(dtype), index=self.index).__finalize__(
            self, method="view"
        )
    ...
    def __array_ufunc__(self, ufunc: Callable, method: str, *inputs: Any, **kwargs: Any):
        ...
        if len(alignable) > 1:
            ...
            for s in alignable[1:]:
                index |= s.index
            inputs = tuple(
                x.reindex(index) if issubclass(t, Series) else x for x, t in zip(inputs, types)
            )
        else:
            index = self.index
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            if isinstance(key, tuple) and not isinstance(self.index, MultiIndex):
                raise ValueError("key of type tuple not found and not a MultiIndex") from err
