...
def lexsort_indexer(keys, orders=None, na_position: str = "last", key: Optional[Callable] = None):
    """
...
def nargsort(
    ...
    if key is not None:
        ...
        return nargsort(items, kind=kind, ascending=ascending, na_position=na_position, key=None)
...
class _KeyMapper:
    ...
    def get_key(self, comp_id):
        return tuple(
            level[table.get_item(comp_id)] for table, level in zip(self.tables, self.levels)
        )
