...
def _get_join_indexers(left_keys, right_keys, sort: bool = False, how: str = "inner", **kwargs):
    """
...
class _AsOfMerge(_OrderedMerge):
    ...
    def _get_join_indexers(self):
        ...
        def flip(xs) -> np.ndarray:
            ...
            xs = [
                x if not is_extension_array_dtype(x) else extract_array(x)._values_for_argsort()
                for x in xs
            ]
...
def _factorize_keys(
    ...
    elif is_categorical_dtype(lk) and is_categorical_dtype(rk) and is_dtype_equal(lk, rk):
