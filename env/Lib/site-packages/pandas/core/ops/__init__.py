...
from pandas._libs.ops_dispatch import (  # noqa:F401
...
def dispatch_to_series(left, right, func, axis: Optional[int] = None):
    ...
    return type(left)._from_arrays(arrays, left.columns, left.index, verify_integrity=False)
...
def _align_method_FRAME(left, right, axis, flex: Optional[bool] = False, level: Level = None):
    """
    ...
    if isinstance(right, np.ndarray):
        ...
        if right.ndim == 1:
            ...
            raise ValueError(
                "Unable to coerce to Series/DataFrame, " f"dimension must be <= 2: {right.shape}"
            )
...
def _frame_arith_method_with_reindex(left: "DataFrame", right: "DataFrame", op) -> "DataFrame":
    """
...
def _arith_method_FRAME(cls: Type["DataFrame"], op, special: bool):
    ...
    def f(self, other, axis=default_axis, level=None, fill_value=None):
        ...
        if _should_reindex_frame_op(self, other, op, axis, default_axis, fill_value, level):
