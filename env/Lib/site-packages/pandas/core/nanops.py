...
def _get_fill_value(dtype: DtypeObj, fill_value: Optional[Scalar] = None, fill_value_typ=None):
...
def _na_for_min_count(values: np.ndarray, axis: Optional[int]) -> Union[Scalar, np.ndarray]:
    """
...
def nanmean(
    ...
    if is_integer_dtype(dtype) or is_datetime64_any_dtype(dtype) or is_timedelta64_dtype(dtype):
        dtype_sum = np.float64
    elif is_float_dtype(dtype):
        dtype_sum = dtype
        dtype_count = dtype
