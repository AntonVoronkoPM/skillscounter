...
def soft_convert_objects(
    ...
    if conversion_count == 0:
        ...
        raise ValueError(
            "Only one of 'datetime', 'numeric' or " "'timedelta' can be True when when coerce=True."
        )
...
def maybe_infer_to_datetimelike(value, convert_dates: bool = False):
    ...
    if isinstance(value, (ABCDatetimeIndex, ABCPeriodIndex, ABCDatetimeArray, ABCPeriodArray)):
        return value
...
def maybe_cast_to_datetime(value, dtype, errors: str = "raise"):
    ...
    if dtype is not None:
        ...
        if is_datetime64 or is_datetime64tz or is_timedelta64:
            ...
            if is_datetime64 and not is_dtype_equal(getattr(dtype, "subtype", dtype), DT64NS_DTYPE):
        ...
        # conversion
            ...
            is_array and not (issubclass(value.dtype.type, np.integer) or value.dtype == np.object_)
        ):
...
def construct_1d_arraylike_from_scalar(value, length: int, dtype: DtypeObj) -> ArrayLike:
    """
