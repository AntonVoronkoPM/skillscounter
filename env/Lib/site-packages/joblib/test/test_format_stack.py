...
import pytest
from joblib.format_stack import (
    _fixed_getframes,
    format_exc,
    format_records,
    safe_repr,
...
def test_format_records():
    try:
        ...
        arrow_regex = r"^-+>\s+\d+\s+"
        assert re.search(
            arrow_regex + r"_raise_exception\('a', 42\)", formatted_records[0], re.MULTILINE
        )
        assert re.search(arrow_regex + r"helper\(a, b\)", formatted_records[1], re.MULTILINE)
