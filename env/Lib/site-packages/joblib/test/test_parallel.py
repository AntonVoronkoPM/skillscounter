...
from joblib.my_exceptions import JoblibException, WorkerInterrupt
...
def test_backend_context_manager(monkeypatch, backend):
    ...
    Parallel(n_jobs=2, backend="threading")(
        delayed(check_backend_context_manager)(b) for b in all_backends_for_context_manager if not b
    )
...
def test_retrieval_context():
    ...
    with parallel_backend("retrieval") as (ba, _):
        Parallel(n_jobs=2)(delayed(nested_call, check_pickle=False)(i) for i in range(5))
        assert ba.i == 1
...
def test_delayed_check_pickle_deprecated():
    ...
    with warns(DeprecationWarning):
        f, args, kwargs = delayed(UnpicklableCallable(), check_pickle=False)("a", option="b")
