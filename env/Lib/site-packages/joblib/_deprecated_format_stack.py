    ...
    for frame, file, lnum, func, lines, index in records:
        ...
        if func == "?":
            ...
            try:
                call = "in %s%s" % (
                    func,
                    inspect.formatargvalues(args, varargs, varkw, locals, formatvalue=eq_repr),
                )
            except KeyError:
        ...
        if index is None:
            ...
            frames.append(
                "%s%s" % (level, "".join(_format_traceback_lines(lnum, index, lines, lvals)))
            )
...
def format_outer_frames(context=5, stack_start=None, stack_end=None, ignore_ipython=True):
    ...
    for i, (frame, filename, line_no, func_name, lines, index) in enumerate(records):
        ...
        if ignore_ipython:
            ...
            if os.path.basename(filename) in ("iplib.py", "py3compat.py") and func_name in (
                "execfile",
                "safe_execfile",
                "runcode",
            ):
