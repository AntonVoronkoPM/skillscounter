...
class Headers(object):
    ...
    def __eq__(self, other):
        ...
        return other.__class__ is self.__class__ and set(map(lowered, other._list)) == set(
            map(lowered, self._list)
        )
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key, (slice, integer_types)):
            if isinstance(key, integer_types):
                value = [value]
            value = [(_unicodify_header_value(k), _unicodify_header_value(v)) for (k, v) in value]
        ...
        return "%s %s" % (
            ...
            ", ".join(
                [
                    ...
                    % (
                        key,
                        quote_header_value(value, allow_token=key not in self._require_quoting),
                    )
