...
_always_safe = frozenset(
    bytearray(b"abcdefghijklmnopqrstuvwxyz" b"ABCDEFGHIJKLMNOPQRSTUVWXYZ" b"0123456789" b"-._~")
)
...
def iri_to_uri(iri, charset="utf-8", errors="strict", safe_conversion=False):
    ...
    return to_native(url_unparse((iri.scheme, iri.encode_netloc(), path, query, fragment)))
...
class Href(object):
    ...
    def __call__(self, *path, **query):
        ...
        path = "/".join(
            [to_unicode(url_quote(x, self.charset), "ascii") for x in path if x is not None]
        ).lstrip("/")
        rv = self.base
        if path:
            if not rv.endswith("/"):
