...
class DebugReprGenerator(object):
    ...
    def dict_repr(self, d, recursive, limit=5):
        ...
        for idx, (key, value) in enumerate(iteritems(d)):
            ...
            buf.append(
                ...
                '<span class="value">%s</span></span>' % (self.repr(key), self.repr(value))
            )
        if have_extended_section:
            buf.append("</span>")
        buf.append("}")
    ...
    def fallback_repr(self):
        ...
        return '<span class="brokenrepr">&lt;broken repr (%s)&gt;' "</span>" % escape(info.strip())
    ...
    def render_object_dump(self, items, title, repr=None):
        ...
        for key, value in items:
            html_items.append("<tr><th>%s<td><pre class=repr>%s</pre>" % (escape(key), value))
        if not html_items:
