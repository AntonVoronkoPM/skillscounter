    ...
    def __init__(self, exc_type, exc_value, tb):
        ...
        if not PY2:
            if exc_value.__cause__ is not None:
                self.info = "The above exception was the direct cause of the" " following exception"
            elif exc_value.__context__ is not None:
                self.info = "During handling of the above exception, another" " exception occurred"
...
class Frame(object):
    ...
    @cached_property
    def is_library(self):
        return any(self.filename.startswith(path) for path in sysconfig.get_paths().values())
    ...
    def sourcelines(self):
        ...
        if source is None:
            try:
                with open(to_native(self.filename, get_filesystem_encoding()), mode="rb") as f:
                    source = f.read()
            except IOError:
                return []
