...
from . import _fitpack, _ppoly, dfitpack, fitpack
...
class PPoly(_PPolyBase):
    ...
    def from_bernstein_basis(cls, bp, extrapolate=None):
        ...
        if not isinstance(bp, BPoly):
            raise TypeError(
                ".from_bernstein_basis only accepts BPoly instances. " "Got %s instead." % type(bp)
            )
...
class BPoly(_PPolyBase):
    ...
    def from_power_basis(cls, pp, extrapolate=None):
        ...
        if not isinstance(pp, PPoly):
            raise TypeError(
                ".from_power_basis only accepts PPoly instances. " "Got %s instead." % type(pp)
            )
    ...
    def __init__(self, points, values, method="linear", bounds_error=True, fill_value=np.nan):
        ...
        for i, p in enumerate(points):
            if not np.all(np.diff(p) > 0.0):
                raise ValueError("The points in dimension %d must be strictly " "ascending" % i)
...
def interpn(points, values, xi, method="linear", bounds_error=True, fill_value=np.nan):
    ...
    for i, p in enumerate(xi.T):
        if bounds_error and not np.logical_and(np.all(grid[i][0] <= p), np.all(p <= grid[i][-1])):
            raise ValueError("One of the requested xi is out of bounds " "in dimension %d" % i)

    # perform interpolation
    if method == "linear":
        interp = RegularGridInterpolator(
            points, values, method="linear", bounds_error=bounds_error, fill_value=fill_value
        )
        return interp(xi)
    elif method == "nearest":
        interp = RegularGridInterpolator(
            points, values, method="nearest", bounds_error=bounds_error, fill_value=fill_value
        )
        return interp(xi)
