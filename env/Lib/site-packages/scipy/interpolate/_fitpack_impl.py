...
def splprep(
    ...
    if ier <= 0 and not quiet:
        warnings.warn(
            RuntimeWarning(_iermess[ier][0] + "\tk=%d n=%d m=%d fp=%f s=%f" % (k, len(t), m, fp, s))
        )
...
def bisplrep(
    ...
    lwrk1 = _int_overflow(
        u * v * (2 + b1 + b2) + 2 * (u + v + km * (m + ne) + ne - kx - ky) + b2 + 1, msg=msg
    )
...
def splder(tck, n=1):
    ...
    if n > k:
        raise ValueError(
            ("Order of derivative (n = %r) must be <= " "order of spline (k = %r)") % (n, tck[2])
        )
