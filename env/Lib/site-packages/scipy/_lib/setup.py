...
def configuration(parent_package="", top_path=None):
    ...
    config.add_extension(
        "_ccallback_c", sources=["_ccallback_c.c"], depends=depends, include_dirs=[include_dir]
    )
    ...
    def get_messagestream_config(ext, build_dir):
        ...
        target = os.path.join(os.path.dirname(__file__), "src", "messagestream_config.h")
    ...
    config.add_extension(
        "_test_deprecation_call", sources=["_test_deprecation_call.c"], include_dirs=[include_dir]
    )

    config.add_extension(
        "_test_deprecation_def", sources=["_test_deprecation_def.c"], include_dirs=[include_dir]
    )
