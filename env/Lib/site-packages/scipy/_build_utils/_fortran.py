...
def get_fcompiler_macro_include_flags(path):
    ...
    flags = {
        ...
        "gnu95": ["-cpp", "-ffree-line-length-none", "-ffixed-line-length-none", "-I" + path],
        "intel": intel_opts,
        "intele": intel_opts,
        "intelem": intel_opts,
        "intelv": intel_opts,
...
def _blas_ilp64_pre_build_hook(cmd, ext, blas_info):
    ...
    if suffix:
        if not suffix.endswith("_"):
            ...
            raise RuntimeError("BLAS/LAPACK has incompatible symbol suffix: " "{!r}".format(suffix))
...
def generic_pre_build_hook(cmd, ext, fcompiler_flags, patch_source_func=None, source_fnpart=None):
    """
    ...
    if is_clib:
        ...
        compilers = [(f77, ext.extra_f77_compile_args), (f90, ext.extra_f90_compile_args)]
    ...
    if patch_source_func is not None:
        if is_clib:
            ...
            new_sources = _generic_patch_sources(ext.sources, patch_source_func, source_fnpart)
