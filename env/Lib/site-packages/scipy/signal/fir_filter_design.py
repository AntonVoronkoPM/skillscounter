...
def kaiserord(ripple, width):
    ...
    if A < 8:
        ...
        raise ValueError(
            "Requested maximum ripple attentuation %f is too " "small for the Kaiser formula." % A
        )
    beta = kaiser_beta(A)
...
def firwin2(
    numtaps, freq, gain, nfreqs=None, window="hamming", nyq=None, antisymmetric=False, fs=None
):
    """
    FIR filter design using the window method.
...
def remez(
    ...
    return sigtools._remez(numtaps, bands, desired, weight, tnum, fs, maxiter, grid_density)
...
def firls(numtaps, bands, desired, weight=None, nyq=None, fs=None):
    ...
    if len(weight) != len(desired):
        raise ValueError(
            "weight must be the same size as the number of " "band pairs (%s)." % (len(bands),)
        )
    if (weight < 0).any():
