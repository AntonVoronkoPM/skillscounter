...
from .base import _ufuncs_with_fixed_point_at_zero, spmatrix
from .sputils import isscalarlike, matrix, validateaxis
    ...
    def astype(self, dtype, casting="unsafe", copy=True):
        ...
        if self.dtype != dtype:
            return self._with_data(
                self._deduped_data().astype(dtype, casting=casting, copy=copy), copy=copy
            )
        elif copy:
            return self.copy()
        else:
...
class _minmax_mixin(object):
    ...
    def _min_or_max_axis(self, axis, min_or_max):
        ...
        if axis == 0:
            return coo_matrix(
                (value, (np.zeros(len(value)), major_index)), dtype=self.dtype, shape=(1, M)
            )
        else:
            return coo_matrix(
                (value, (major_index, np.zeros(len(value)))), dtype=self.dtype, shape=(M, 1)
            )
