...
def configuration(parent_package="", top_path=None):
    ...
    atlas_version = (
        [v[3:-3] for k, v in lapack_opt.get("define_macros", []) if k == "ATLAS_INFO"] + [None]
    )[0]
    if atlas_version:
    ...
    # _flinalg:
    config.add_extension(
        "_flinalg", sources=[join("src", "det.f"), join("src", "lu.f")], extra_info=lapack_opt
    )
    ...
    # _solve_toeplitz:
    config.add_extension(
        "_solve_toeplitz", sources=[("_solve_toeplitz.c")], include_dirs=[get_numpy_include_dirs()]
    )
    ...
    config.add_extension(
        ...
        depends=["cython_blas.pyx", "cython_blas.pxd", "fortran_defs.h", "_blas_subroutines.h"],
        include_dirs=["."],
        libraries=["fwrappers"],
        extra_info=lapack_opt,
    )
