...
import numpy as np
import pytest
from numpy import finfo, isclose, nan, power
...
from scipy.optimize import newton, root_scalar, zeros

# Import testing parameters
from scipy.optimize._tstutils import fstrings as tstutils_fstrings
...
class TestBasic(object):
    def run_check_by_name(self, name, smoothness=0, **kwargs):
        ...
        for function, fname in zip(tstutils_functions, tstutils_fstrings):
            if smoothness > 0 and fname in ["f4", "f5", "f6"]:
                continue
            r = root_scalar(
                function, method=name, bracket=[a, b], x0=a, xtol=xtol, rtol=rtol, **kwargs
            )
            zero = r.root
            assert_(r.converged)
            assert_allclose(
                zero, 1.0, atol=xtol, rtol=rtol, err_msg="method %s, function %s" % (name, fname)
            )
    ...
    def test_array_newton_zero_der_failures(self):
        ...
        assert_warns(
            RuntimeWarning, zeros.newton, lambda y: y**2 - 2, [0.0, 0.0], lambda y: 2 * y
        )
