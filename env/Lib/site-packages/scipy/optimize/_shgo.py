...
class SHGO(object):
    ...
    def iterate_hypercube(self):
        ...
        if self.n_sampled == 0:
            ...
            self.HC = Complex(
                self.dim, self.func, self.args, self.symmetry, self.bounds, self.g_cons, self.g_args
            )
        else:
    ...
    def minimise_pool(self, force_iter=False):
        ...
        while not self.stop_l_iter:
            ...
            if self.local_iter is not None:
                if self.disp:
                    logging.info(
                        "SHGO.iters in function minimise_pool = {}".format(self.local_iter)
                    )
                self.local_iter -= 1
        ...
        for j in range(1, D):
            ...
            if L <= s:
                ...
                for i in range(s + 1, L + 1):
                    ...
                    for k in range(1, s):
                        V[i] ^= np.array((((a >> (s - 1 - k)) & 1) * V[i - k]), dtype=unsigned)
