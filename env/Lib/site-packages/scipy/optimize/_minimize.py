...
def minimize(
    ...
    **Custom minimizers**
    ...
    # - hessp
    if (
        meth not in ("newton-cg", "dogleg", "trust-ncg", "trust-constr", "trust-krylov", "_custom")
        and hessp is not None
    ):
        warn(
            "Method %s does not use Hessian-vector product " "information (hessp)." % method,
            RuntimeWarning,
        )
    # - constraints or bounds
    if meth in ("nelder-mead", "cg", "bfgs", "newton-cg", "dogleg", "trust-ncg") and (
    ...
    if callable(jac):
        ...
        fun = MemoizeJac(fun)
        jac = fun.derivative
    elif jac in FD_METHODS and meth in ["trust-constr", "bfgs", "cg", "l-bfgs-b", "tnc"]:
        # finite differences
        pass
    ...
    if meth == "nelder-mead":
        ...
        return _minimize_trust_krylov(fun, x0, args, jac, hess, hessp, callback=callback, **options)
