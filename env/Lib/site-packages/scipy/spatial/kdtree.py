...
class KDTree(object):
    ...
    def query(self, x, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        ...
        if np.shape(x)[-1] != self.m:
            raise ValueError(
                "x must consist of vectors of length %d but has shape %s" % (self.m, np.shape(x))
            )
        if p < 1:
    ...
    def query_pairs(self, r, p=2.0, eps=0):
        ...
        traverse_checking(
            self.tree, Rectangle(self.maxes, self.mins), self.tree, Rectangle(self.maxes, self.mins)
        )
        return results
