...
class FitDataError(ValueError):
    ...
    def __init__(self, distr, lower, upper):
        self.args = (
            ...
            "< {upper!r} for each x in `data`.".format(distr=distr, lower=lower, upper=upper),
        )
...
class betaprime_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        if n == 1.0:
            ...
            return np.where(
                b > 3, a * (a + 1.0) * (a + 2.0) / ((b - 3.0) * (b - 2.0) * (b - 1.0)), np.inf
            )
        elif n == 4.0:
            return np.where(
                b > 4,
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        ku1 = _lazywhere(
            c >= -1.0 / 4,
            (g1, g2, g3, g4, g2mg12),
            lambda g1, g2, g3, g4, g2mg12: (g4 + (-4 * g3 + 3 * (g2 + g2mg12) * g1) * g1)
            / g2mg12**2,
            fillvalue=np.nan,
        )
...
class laplace_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        _check_fit_input_parameters(data, args, kwds, fixed_param=(floc, fscale))
...
class levy_stable_gen(rv_continuous):
    ...
    def _rvs(self, alpha, beta, size=None, random_state=None):
        ...
        def alphanot1func(alpha, beta, TH, aTH, bTH, cosTH, tanTH, W):
            res = _lazywhere(
                beta == 0, (alpha, beta, TH, aTH, bTH, cosTH, tanTH, W), beta0func, f2=otherwise
            )
            return res
        ...
        tanTH = np.tan(TH)
        res = _lazywhere(
            alpha == 1, (alpha, beta, TH, aTH, bTH, cosTH, tanTH, W), alpha1func, f2=alphanot1func
        )
        return res
    ...
    def _pdf_single_value_cf_integrate(x, alpha, beta):
        ...
        return (
            integrate.quad(
                lambda t: np.real(np.exp(-1j * t * x) * cf(t)), -np.inf, np.inf, limit=1000
            )[0]
            / np.pi
            / 2
        )
    ...
    def _pdf_single_value_zolotarev(x, alpha, beta):
        ...
        if alpha != 1:
            ...
            if x0 > zeta:
                ...
                def g(theta):
                    return V(theta) * np.real(np.complex128(x0 - zeta) ** (alpha / (alpha - 1)))
            ...
            if beta != 0:
                ...
                def V(theta):
                    ...
                    return (
                        2.0 * expr_1 * np.exp(expr_1 * np.tan(theta) / beta) / np.cos(theta) / np.pi
                    )
...
class lognorm_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        for name in ["f0", "fs", "fix_s", "floc", "fscale", "loc", "scale", "optimizer"]:
            kwds.pop(name, None)
        if kwds:
            raise TypeError("Unknown arguments: %s." % kwds)
...
class nct_gen(rv_continuous):
    ...
    def _pdf(self, x, df, nc):
        ...
        trm1 -= n * np.log(2) + nc * nc / 2.0 + (n / 2.0) * np.log(fac1) + sc.gammaln(n / 2.0)
        Px = np.exp(trm1)
        valF = ncx2 / (2 * fac1)
...
class powerlognorm_gen(rv_continuous):
    ...
    def _pdf(self, x, c, s):
        ...
        #                                         (Phi(-log(x)/s))**(c-1),
        return (
            c / (x * s) * _norm_pdf(np.log(x) / s) * pow(_norm_cdf(-np.log(x) / s), c * 1.0 - 1.0)
        )
    ...
    with np.errstate(invalid="ignore"):
        ...
        if np.any(cond_inner):
            ...
            if delta > 0:
                ...
                np.place(out, cond_inner, np.exp(_truncnorm_logpdf_scalar(x[cond_inner], a, b)))
        return out[0] if (shp == ()) else out
...
def _truncnorm_logcdf_scalar(x, a, b):
    with np.errstate(invalid="ignore"):
        ...
        if np.any(cond_inner):
            delta = _truncnorm_get_delta_scalar(a, b)
            if delta > 0:
                np.place(out, cond_inner, np.log((_norm_cdf(x[cond_inner]) - _norm_cdf(a)) / delta))
            else:
...
def _truncnorm_cdf_scalar(x, a, b):
    with np.errstate(invalid="ignore"):
        ...
        if np.any(cond_inner):
            delta = _truncnorm_get_delta_scalar(a, b)
            if delta > 0:
                np.place(out, cond_inner, (_norm_cdf(x[cond_inner]) - _norm_cdf(a)) / delta)
            else:
                with np.errstate(divide="ignore"):
                    np.place(out, cond_inner, np.exp(_truncnorm_logcdf_scalar(x[cond_inner], a, b)))
        return out[0] if (shp == ()) else out
...
def _truncnorm_ppf_scalar(q, a, b):
    ...
    if np.any(cond_inner):
        ...
        if delta > 0:
            ...
            np.place(out, cond_inner, -_norm_ilogcdf(np.log1p(-qinner) + _norm_logsf(a)))
        elif np.isinf(a):
            ...
            if b < 0:
                ...
                x = [
                    optimize.zeros.brentq(_f_sf, a, b, args=(c,), maxiter=TRUNCNORM_MAX_BRENT_ITERS)
                    for c in values
                ]
                np.place(out, cond_inner, x)
        out[out < a] = a
...
class argus_gen(rv_continuous):
    ...
    def _rvs_scalar(self, chi, numsamples=None, random_state=None):
        ...
        if chi <= 2.611:
            ...
            # use ratio of uniforms method
            def f(x):
                return np.where((x >= 0) & (x <= chi), np.exp(2 * np.log(x) - x**2 / 2), 0)

            umax = np.sqrt(2) / np.exp(0.5)
            vmax = 4 / np.exp(1)
            z = rvs_ratio_uniforms(f, umax, 0, vmax, size=numsamples, random_state=random_state)
