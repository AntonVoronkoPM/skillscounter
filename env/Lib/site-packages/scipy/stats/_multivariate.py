...
from scipy.special import entr, gammaln, multigammaln, psi, xlogy
...
def _dirichlet_check_parameters(alpha):
    ...
    if np.min(alpha) <= 0:
        ...
        raise ValueError(
            "Parameter vector 'a' must be one dimensional, " "but a.shape = %s." % (alpha.shape,)
        )
    return alpha
...
class unitary_group_gen(multi_rv_generic):
    ...
    def rvs(self, dim, size=1, random_state=None):
        ...
        z = (
            1
            / math.sqrt(2)
            * (random_state.normal(size=(dim, dim)) + 1j * random_state.normal(size=(dim, dim)))
        )
