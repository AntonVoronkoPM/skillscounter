...
class TestFFT1D(object):
    ...
    def test_fft2(self):
        ...
        assert_array_almost_equal(fft.fft2(x) / np.sqrt(30 * 20), fft.fft2(x, norm="ortho"))

    def test_ifft2(self):
        x = random((30, 20)) + 1j * random((30, 20))
        assert_array_almost_equal(fft.ifft(fft.ifft(x, axis=1), axis=0), fft.ifft2(x))
        assert_array_almost_equal(fft.ifft2(x) * np.sqrt(30 * 20), fft.ifft2(x, norm="ortho"))

    def test_fftn(self):
        x = random((30, 20, 10)) + 1j * random((30, 20, 10))
        assert_array_almost_equal(fft.fft(fft.fft(fft.fft(x, axis=2), axis=1), axis=0), fft.fftn(x))
        assert_array_almost_equal(fft.fftn(x) / np.sqrt(30 * 20 * 10), fft.fftn(x, norm="ortho"))
    ...
    def test_rfft(self):
        ...
        for n in [x.size, 2 * x.size]:
            for norm in [None, "ortho"]:
                assert_array_almost_equal(
                    fft.fft(x, n=n, norm=norm)[: (n // 2 + 1)], fft.rfft(x, n=n, norm=norm)
                )
    ...
    def test_rfft2(self):
        ...
        assert_array_almost_equal(fft.rfft2(x) / np.sqrt(30 * 20), fft.rfft2(x, norm="ortho"))

    def test_irfft2(self):
        x = random((30, 20))
        assert_array_almost_equal(x, fft.irfft2(fft.rfft2(x)))
        assert_array_almost_equal(x, fft.irfft2(fft.rfft2(x, norm="ortho"), norm="ortho"))

    def test_rfftn(self):
        x = random((30, 20, 10))
        assert_array_almost_equal(fft.fftn(x)[:, :, :6], fft.rfftn(x))
        assert_array_almost_equal(fft.rfftn(x) / np.sqrt(30 * 20 * 10), fft.rfftn(x, norm="ortho"))

    def test_irfftn(self):
        x = random((30, 20, 10))
        assert_array_almost_equal(x, fft.irfftn(fft.rfftn(x)))
        assert_array_almost_equal(x, fft.irfftn(fft.rfftn(x, norm="ortho"), norm="ortho"))
    ...
    def test_hfft(self):
        ...
        assert_array_almost_equal(fft.hfft(x_herm) / np.sqrt(30), fft.hfft(x_herm, norm="ortho"))
    ...
    def test_ihfft(self):
        ...
        assert_array_almost_equal(x_herm, fft.ihfft(fft.hfft(x_herm, norm="ortho"), norm="ortho"))

    def test_hfft2(self):
        x = random((30, 20))
        assert_array_almost_equal(x, fft.hfft2(fft.ihfft2(x)))
        assert_array_almost_equal(x, fft.hfft2(fft.ihfft2(x, norm="ortho"), norm="ortho"))

    def test_ihfft2(self):
        x = random((30, 20))
        assert_array_almost_equal(fft.ifft2(x)[:, :11], fft.ihfft2(x))
        assert_array_almost_equal(fft.ihfft2(x) * np.sqrt(30 * 20), fft.ihfft2(x, norm="ortho"))

    def test_hfftn(self):
        x = random((30, 20, 10))
        assert_array_almost_equal(x, fft.hfftn(fft.ihfftn(x)))
        assert_array_almost_equal(x, fft.hfftn(fft.ihfftn(x, norm="ortho"), norm="ortho"))
...
@pytest.mark.parametrize(
    "dtype", [np.float32, np.float64, np.longfloat, np.complex64, np.complex128, np.longcomplex]
)
