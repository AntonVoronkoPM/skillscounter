...
def generate_loop(func_inputs, func_outputs, func_retval, ufunc_inputs, ufunc_outputs):
    ...
    for j in range(len(func_inputs)):
        ...
        fvars.append(
            "<%s>(<%s*>ip%d)[0]" % (CY_TYPES[func_inputs[j]], CY_TYPES[ufunc_inputs[j]], j)
        )
...
class Func(object):
    ...
    def get_prototypes(self, nptypes_for_h=False):
        ...
        for func_name, inarg, outarg, ret, header in self.signatures:
            ...
            cy_args = [CY_TYPES[x] for x in inarg] + [CY_TYPES[x] + " *" for x in outarg]
    ...
    def cython_func_name(self, c_name, specialized=False, prefix="_func_", override=True):
