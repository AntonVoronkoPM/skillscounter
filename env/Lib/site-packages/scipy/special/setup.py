...
try:
    ...
    raise ValueError(
        "numpy >= 1.4 is required (detected %s from %s)" % (numpy.__version__, numpy.__file__)
    )
...
def configuration(parent_package="", top_path=None):
    ...
    ufuncs_dep = headers + ufuncs_src + amos_src + cephes_src + mach_src + cdf_src + specfun_src
    ...
    config.add_extension(
        "_ufuncs", depends=ufuncs_dep, sources=ufuncs_src, extra_info=get_info("npymath"), **cfg
    )
