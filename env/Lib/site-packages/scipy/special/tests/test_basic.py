...
class TestCephes(object):
    ...
    def test_betaincinv(self):
        ...
        assert_allclose(
            cephes.betaincinv(0.0342, 171, 0.25), 8.4231316935498957e-21, rtol=3e-12, atol=0
        )
...
class TestCombinatorics(object):
    ...
    def test_comb_with_np_int64(self):
        ...
        np_n = np.int64(n)
        np_k = np.int64(k)
        assert_equal(special.comb(np_n, np_k, exact=True), special.comb(n, k, exact=True))
...
class TestFactorialFunctions(object):
    def test_factorial(self):
        ...
        assert_equal(
            special.factorial([7, 4, 15, 10], exact=True), [5040, 24, 1307674368000, 3628800]
        )
        ...
        # object arrays
        assert_equal(
            special.factorial(np.arange(-3, 22), True), special.factorial(np.arange(-3, 22), False)
        )

        # int64 array
        assert_equal(
            special.factorial(np.arange(-3, 15), True), special.factorial(np.arange(-3, 15), False)
        )

        # int32 array
        assert_equal(
            special.factorial(np.arange(-3, 5), True), special.factorial(np.arange(-3, 5), False)
        )
...
class TestBessel(object):
    ...
    def _cephes_vs_amos_points(self):
        ...
        for v in [-120, -100.3, -20.0, -10.0, -1.0, -0.5, 0.0, 1.0, 12.49, 120.0, 301]:
            for z in [-1300, -11, -10, -1, 1.0, 10.0, 200.5, 401.0, 600.5, 700.6, 1300, 10003]:
                yield v, z
...
class TestLegendreFunctions(object):
    ...
    def test_lpmn(self):
        ...
        assert_array_almost_equal(
            lp, (array([[1.00000, 0.50000, -0.12500]]), array([[0.00000, 1.00000, 1.50000]])), 4
        )

    def test_lpn(self):
        lpnf = special.lpn(2, 0.5)
        assert_array_almost_equal(
            lpnf, (array([1.00000, 0.50000, -0.12500]), array([0.00000, 1.00000, 1.50000])), 4
        )
