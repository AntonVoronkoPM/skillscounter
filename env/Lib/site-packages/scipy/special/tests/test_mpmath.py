...
class TestSystematic(object):
    ...
    def test_besselk(self):
        assert_mpmath_equal(
            sc.kv, mpmath.besselk, [Arg(-200, 200), Arg(0, np.inf)], nan_ok=False, rtol=1e-12
        )

    def test_besselk_int(self):
        assert_mpmath_equal(
            sc.kn, mpmath.besselk, [IntArg(-200, 200), Arg(0, np.inf)], nan_ok=False, rtol=1e-12
        )
    ...
    def test_bessely(self):
        ...
        assert_mpmath_equal(
            sc.yv, exception_to_nan(mpbessely), [Arg(-1e100, 1e100), Arg(-1e8, 1e8)], n=5000
        )
    ...
    def test_beta(self):
        ...
        assert_mpmath_equal(
            sc.beta, beta, np.array(bad_points), dps=400, ignore_inf_sign=True, atol=1e-11
        )
    ...
    def test_chi_complex(self):
        ...
        assert_mpmath_equal(
            chi, mpmath.chi, [ComplexArg(complex(-np.inf, -1e8), complex(np.inf, 1e8))], rtol=1e-12
        )
    ...
    def test_ci_complex(self):
        ...
        assert_mpmath_equal(
            ci, mpmath.ci, [ComplexArg(complex(-1e8, -np.inf), complex(1e8, np.inf))], rtol=1e-8
        )
    ...
    def test_expm1_complex(self):
        ...
        assert_mpmath_equal(
            sc.expm1, mpmath.expm1, [ComplexArg(complex(-np.inf, -1e7), complex(np.inf, 1e7))]
        )
    ...
    def test_ellipfun_sn(self):
        ...
        assert_mpmath_equal(
            lambda u, m: sc.ellipj(u, m)[0], sn, [Arg(-1e6, 1e6), Arg(a=0, b=1)], rtol=1e-8
        )
    ...
    def test_expint(self):
        assert_mpmath_equal(
            sc.expn, mpmath.expint, [IntArg(0, 200), Arg(0, np.inf)], rtol=1e-13, dps=160
        )
    ...
    def test_gegenbauer(self):
        assert_mpmath_equal(
            sc.eval_gegenbauer, exception_to_nan(mpmath.gegenbauer), [Arg(-1e3, 1e3), Arg(), Arg()]
        )
    ...
    def test_hyp0f1(self):
        ...
        assert_mpmath_equal(
            sc.hyp0f1, lambda a, x: mpmath.hyp0f1(a, x, **KW), [Arg(-1e7, 1e7), Arg(0, 1e5)], n=5000
        )
    ...
    def test_igam_fac(self):
        ...
        assert_mpmath_equal(
            _igam_fac, mp_igam_fac, [Arg(0, 1e14, inclusive_a=False), Arg(0, 1e14)], rtol=1e-10
        )
    ...
    def test_pcfd(self):
        ...
        assert_mpmath_equal(
            pcfd, exception_to_nan(lambda v, x: mpmath.pcfd(v, x, **HYPERKW)), [Arg(), Arg()]
        )

    @pytest.mark.xfail(
        run=False, reason="it's not the same as the mpmath function --- maybe different definition?"
    )
    def test_pcfv(self):
        def pcfv(v, x):
    ...
    def test_shi_complex(self):
        ...
        assert_mpmath_equal(
            shi, mpmath.shi, [ComplexArg(complex(-np.inf, -1e8), complex(np.inf, 1e8))], rtol=1e-12
        )
    ...
    def test_si_complex(self):
        ...
        assert_mpmath_equal(
            si, mpmath.si, [ComplexArg(complex(-1e8, -np.inf), complex(1e8, np.inf))], rtol=1e-12
        )
    ...
    def test_struveh(self):
        assert_mpmath_equal(
            sc.struve, exception_to_nan(mpmath.struveh), [Arg(-1e4, 1e4), Arg(0, 1e4)], rtol=5e-10
        )
