...
_SUPPORTED_MODULES = {
    ...
    "_BLISModule": {"user_api": "blas", "internal_api": "blis", "filename_prefixes": ("libblis",)},
}
...
_ALL_PREFIXES = [prefix for m in _SUPPORTED_MODULES.values() for prefix in m["filename_prefixes"]]
_ALL_BLAS_LIBRARIES = [
...
class threadpool_limits:
    ...
    def get_original_num_threads(self):
        ...
        for user_api in self._user_api:
            limits = [
                module.num_threads for module in original_limits.get_modules("user_api", user_api)
            ]
            limits = set(limits)
            n_limits = len(limits)
        ...
        try:
            ...
            while True:
                ...
                if not ps_api.EnumProcessModulesEx(
                    h_process, ctypes.byref(buf), buf_size, ctypes.byref(needed), LIST_MODULES_ALL
                ):
...
class _OpenBLASModule(_Module):
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(self._dynlib, "openblas_set_num_threads", lambda num_threads: None)
...
class _BLISModule(_Module):
    ...
    def get_version(self):
        get_version_ = getattr(self._dynlib, "bli_info_get_version_str", lambda: None)
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(self._dynlib, "bli_thread_set_num_threads", lambda num_threads: None)
        return set_func(num_threads)
...
class _MKLModule(_Module):
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(self._dynlib, "MKL_Set_Num_Threads", lambda num_threads: None)
        return set_func(num_threads)
    ...
    def get_threading_layer(self):
        ...
        set_threading_layer = getattr(self._dynlib, "MKL_Set_Threading_Layer", lambda layer: -1)
...
class _OpenMPModule(_Module):
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(self._dynlib, "omp_set_num_threads", lambda num_threads: None)
        return set_func(num_threads)
